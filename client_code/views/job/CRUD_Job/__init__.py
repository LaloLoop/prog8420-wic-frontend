...
class CRUD_Job(CRUD_JobTemplate):
  ...
  def form_show(self, **event_args):
    ...
    display_fields = ['title', 'speciality']
    
    # populate data grid
    table_columns = ['Title', 'Speciality']
    
    table_rows = [] # repeating_panel takes a list of dictionaries/rows
    for _id in [e['id'] for e in sorted(resp, key = lambda x: x['title'])]:
    ...
    list_of_display_name_tuples = sorted( \
      [(entity_id_to_fields[_id]['title'] + ", " + entity_id_to_fields[_id]['speciality'], _id)
