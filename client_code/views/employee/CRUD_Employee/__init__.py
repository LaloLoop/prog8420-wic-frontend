...
class CRUD_Employee(CRUD_EmployeeTemplate):
  ...
  def form_show(self, **event_args):
    ...
    display_fields = ['person_display_name', 'job_display_name','is_active',
                      'is_superuser','is_verified']
    
    # populate data grid
    table_columns = ['Employee', 'Job_Title', 'Is Active', 'Is SuperUser', 'Is Verified']
    
    table_rows = [] # repeating_panel takes a list of dictionaries/rows
    for _id in [e['id'] for e in sorted(resp, key = lambda x: x['person_display_name'])]:
      fields_dict = entity_id_to_fields[str(_id)]
      table_rows.append({col:fields_dict[f] for col,f in zip(table_columns,display_fields)})
  
    grid_col_widths = [200,100,100,100,100] 
    grid_cols=[{'id':col,
    ...
    list_of_display_name_tuples = sorted( \
      [(entity_id_to_fields[_id]['person_display_name'], _id)
